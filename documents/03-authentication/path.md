...
**Topic:** Day 5 Learning — JWT/OAuth2 flows, token refresh, django-auth
**Traces To:** `03-api/02-api/05-auth/01-auth-flow.md`, `02-architecture/02-security-architecture/auth-design.md`

There are 5 layers. Learn them in order. Each one builds on the previous.
...
Come back after each layer. Answer the questions out loud or in writing.
Each layer must be clear before moving to the next.
After all 5 layers, read [01-auth-flow.md](../../03-api/02-api/05-auth/01-auth-flow.md) top to bottom — every line will make sense.
